var onDocumentLoad = function ( event ) {
	...
	text = text.replace(/<h2>Example<\/h2>/gi, '<h2>示例代码</h2>');
	text = text.replace(/<h2>Constructor<\/h2>/gi, '<h2>构造函数</h2>');
	text = text.replace(/<h2>Properties<\/h2>/gi, '<h2>属性</h2>');
	text = text.replace(/<h2>Methods<\/h2>/gi, '<h2>方法</h2>');
	text = text.replace(/<h2>Source<\/h2>/gi, '<h2>源代码</h2>');
